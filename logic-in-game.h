...
int is_snake_point(const object_t *snake, int x, int y)
{
	...
	{
		if(snake[i].location -> x == x 
			&& snake[i].location -> y == y)
			return TRUE;
		i++;
	}
...
int is_eat_fruit(const object_t *snake, location_t fruit)
{
	return ((snake[0].location -> x == fruit.x) 
		&& (snake[0].location -> y == fruit.y));
}
...
int is_game_over(const object_t *snake)
{
	...
	while(i < MAX_LENGTH_SNAKE && snake[i].isExist)
	{
		if(headPointX == snake[i].location -> x 
			&& headPointY == snake[i].location -> y)
			return TRUE;
